...
export const updateUser = async (data, onHide, dispatch) => {
  try {
    const res = await axios.patch(`/users/${localStorage.user_id}`, data, {
      withCredentials: true,
    });
...
export const followUser = async (user, dispatch) => {
  try {
    await axios.post(`/users/${user}/follows`, {
      withCredentials: true,
    });
...
export const unFollowUser = async (user, dispatch) => {
  try {
    await axios.delete(`/users/${user}/follows/unfollow`, {
      withCredentials: true,
    });
...
export const suggestions = async () => {
  try {
    return await axios.get("/users/suggestions", { withCredentials: true });
  } catch (error) {
    console.log(error);
  }
};
...
export const getFolllowing = async (id, dispatch, param) => {
  try {
    dispatch(request());
    const res = await axios.get(`/users/${id}/follows`, {
      withCredentials: true,
      params: {
        option: param,
      },
